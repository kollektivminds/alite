...
# The test function requests our fixtures: the mocked api_client and the db_session
def test_exercise_generation_respects_valid_id_ranges(api_client, db_session):
    ...
    # Ensure our database clone actually contains target structural data
    assert (
        min_lem_id is not None
    ), "Lexicon table must contain rows for integration tests."
